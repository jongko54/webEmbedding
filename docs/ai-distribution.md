...
- Remote intake endpoint: `https://webembedding-jongkos-mcp.vercel.app/mcp`
...
The hosted Apps SDK intake endpoint is deployed at `https://webembedding-jongkos-mcp.vercel.app/mcp`. It intentionally exposes only low-risk read-only/routing tools: `detect_runtime_capabilities`, `inspect_url`, `discover_embed_candidates`, `classify_clone_mode`, `generate_embed_snippet`, and `plan_reproduction_path`.
